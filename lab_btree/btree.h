...
size_t inserthelper(const std::vector< T >& elements, const C& val, size_t low, size_t high){
    
    //get median
	size_t median = (high+low)/2;

    //base case
    if(median == 0 && val < elements[median]){ return 0; }
    if(median == size_t(elements.size()) - 1 && val > elements[median]) { return size_t(elements.size()); }
    if(val == elements[median]) { return median; }
	if(val > elements[median] && val < elements[median+1]){ return median+1; }

    //recursion step
	if(val < elements[median]){
        return inserthelper(elements, val, low, median);
    }
	else{
        return inserthelper(elements, val, median + 1, high);   
    }
}
...
size_t insertion_idx(const std::vector< T >& elements, const C& val)
{
	...
	return inserthelper(elements, val, 0, size_t(elements.size())-1);
}

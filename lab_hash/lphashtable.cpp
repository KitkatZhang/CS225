...
void LPHashTable<K, V>::remove(K const& key)
{
     ...
     */
    size_t index = hash(key, size);
    while(should_probe[index] == 1 && table[index]->first != key){
        index = (index + 1)%size;
    }
    if(should_probe[index] == 0){
        return;
    }
...
void LPHashTable<K, V>::resizeTable()
{
     ...
     */
    pair<K, V>** temp;
    temp = new pair<K, V>*[findPrime(2*size)];
    bool* temp1;
    temp1 = new bool[findPrime(2*size)];
    ...
    for(size_t i = 0; i < size; i++){
        if(table[i] != NULL){
            size_t index = hash(table[i]->first, findPrime(2*size));
            while(temp1[index] == true){
                index = (index + 1)%findPrime(2*size);
            }
            temp[index] =  new pair<K, V>(*(table[i]));
            temp1[index] = true;
        }
    }
    for (size_t i = 0; i < size; i++){
